...
import { AnswerSheet, UserProfile, QuestionPaper } from '../types';
    ...
    return (
      ...
      >
        <div className="text-center">
          ...
          <h2 className="mt-4 font-syne text-3xl font-extrabold text-white">Answer <span className="text-[#38bdf8]">Keys</span></h2>
          <p className="mt-2 font-instrument text-[#64748b]">Review question papers and their AI-generated answer keys.</p>
        </div>

        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          <div className="lg:col-span-1 space-y-4">
            <h3 className="font-syne text-xl font-bold text-white">Question Papers</h3>
            {loading ? (
              <p className="text-sm text-[#64748b]">Loading papers...</p>
            ) : papers.length === 0 ? (
              <p className="text-sm text-[#64748b]">No papers found.</p>
            ) : (
              papers.map(paper => (
                <button
                  key={paper.id}
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-syne text-sm font-bold text-white">{paper.title}</h4>
                      <p className="text-[10px] text-[#64748b]">{paper.subject}</p>
                    </div>
                  </div>
                </button>
              ))
            ...
            {selectedPaper ? (
              ...
              >
                <div className="mb-6 flex items-center justify-between">
                  <h3 className="font-syne text-2xl font-bold text-white">{selectedPaper.title} - Answer Key</h3>
                  {selectedPaper.answerKeyUrl && (
                    <a
                      href={selectedPaper.answerKeyUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-2 rounded-xl bg-[#38bdf8] px-4 py-2 font-syne text-xs font-bold text-[#050a12] transition-all hover:bg-[#7dd3fc]"
                    >
                      <FileText size={14} />
                      Download PDF Key
                    </a>
                ...
                <div className="space-y-6">
                  <div className="rounded-2xl bg-white/5 p-6">
                    <h4 className="mb-4 font-syne text-sm font-bold text-[#38bdf8]">Questions</h4>
                    <div className="space-y-2 text-sm text-[#e2e8f0]">
                      {selectedPaper.questions.map((q, i) => (
                        <p key={i} className="border-b border-white/5 pb-2">{q}</p>
                      ))}
                    </div>
                  </div>

                  <div className="rounded-2xl bg-[#38bdf8]/5 p-6">
                    <h4 className="mb-4 font-syne text-sm font-bold text-[#38bdf8]">AI Generated Answer Key</h4>
                    <div className="prose prose-invert max-w-none text-sm text-[#e2e8f0]">
                      <ReactMarkdown>{selectedPaper.answerKey}</ReactMarkdown>
                    </div>
                  </div>
                </div>
              </motion.div>
              ...
              <div className="flex h-full min-h-[400px] flex-col items-center justify-center rounded-3xl border border-dashed border-white/10 bg-white/5 text-center">
                <FileText size={48} className="mb-4 text-[#64748b]" />
                <h3 className="font-syne text-lg font-bold text-white">Select a Paper</h3>
                <p className="mt-2 text-sm text-[#64748b]">Choose a question paper to view its questions and answer key.</p>
              </div>
            )}
          </div>
        </div>
