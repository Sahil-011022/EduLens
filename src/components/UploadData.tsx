...
import { extractQuestionsAndAnswerKey, gradeAnswerSheet } from '../lib/gemini';
...
export default function UploadData({ user }: UploadDataProps) {
  ...
  const handleUploadPaper = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("Extracting questions and answer key with AI...");
      const base64 = await fileToBase64(paperFile);
      const data = await extractQuestionsAndAnswerKey(base64, paperFile.type);
      console.log("AI Extraction complete:", data);

      // 3. Generate PDF Answer Key
      console.log("Generating PDF Answer Key...");
      try {
        const docPDF = new jsPDF();
        docPDF.setFont("helvetica", "bold");
        docPDF.setFontSize(22);
        docPDF.text("EduLens Answer Key", 105, 20, { align: "center" });
        
        docPDF.setFontSize(14);
        docPDF.setFont("helvetica", "normal");
        docPDF.text(`Title: ${paperTitle}`, 20, 40);
        docPDF.text(`Subject: ${paperSubject}`, 20, 50);
        docPDF.text(`Date: ${new Date().toLocaleDateString()}`, 20, 60);
        docPDF.line(20, 65, 190, 65);
        
        docPDF.setFontSize(12);
        const splitText = docPDF.splitTextToSize(data.answerKey, 170);
        ...
        console.log("PDF Blob generated, uploading to storage...");
        const pdfRef = ref(storage, `answerKeys/${Date.now()}_answer_key.pdf`);
        await uploadBytes(pdfRef, pdfBlob);
        const answerKeyUrl = await getDownloadURL(pdfRef);
        console.log("PDF Answer Key uploaded successfully:", answerKeyUrl);
        ...
        // Fallback: save without PDF if PDF fails
        await addDoc(collection(db, 'questionPapers'), {
          title: paperTitle,
          subject: paperSubject,
          fileUrl,
      ...
      console.error("Error in handleUploadPaper:", err);
      showToast(`Error: ${err.message || 'Failed to upload paper'}`, 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUploadAnswer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. AI Grading
      console.log("Grading answer sheet with AI...");
      const base64 = await fileToBase64(answerFile);
      const grading = await gradeAnswerSheet(base64, answerFile.type, paper.questions, paper.answerKey);
      console.log("Grading complete:", grading);
              ...
              papers.map(p => (
                <div key={p.id} className="rounded-2xl border border-white/5 bg-[#0b1220] p-4">
                  ...
                  <div className="mt-4 space-y-2">
                    ...
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-wider text-[#64748b]">Answer Key:</p>
                      <div className="max-h-24 overflow-y-auto rounded-lg bg-[#38bdf8]/10 p-2 text-[10px] text-[#38bdf8]">
                        <ReactMarkdown>{p.answerKey}</ReactMarkdown>
                      </div>
                    </div>
                  </div>
                </div>
